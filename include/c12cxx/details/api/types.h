...
#define RESULT_FROM_ERRNO(x) \
    ((long)(x) <= 0 ? ((long)(x)) : ((long)(((x) & 0x0000FFFF) | (BASE_ERRNO << 16) | 0x80000000)))
...
inline void tVarInit(tVariant* tvar)
{
    assert(tvar != NULL);
...
#define DATA_SET_BEGIN(data_) tVarInit(data_);

#define DATA_SET_END(data_, type_) TV_VT(data_) = type_;
