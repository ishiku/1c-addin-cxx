...
{
    try {
        return reinterpret_cast<const WCHAR_T*>(c12cxx::FactoryRegistry::instance().listNames().data());
    } catch (...) {
        return nullptr;
    }
}

EXPORT long GetClassObject(const WCHAR_T* clsName, IComponentBase** pInterface)
{
    if (!*pInterface) {
        try {
            auto cls_name = std::u16string(reinterpret_cast<const char16_t*>(clsName));
            *pInterface = c12cxx::FactoryRegistry::instance().create(cls_name);
